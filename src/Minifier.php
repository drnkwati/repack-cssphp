...
namespace Repack\CssPhp;

// https://github.com/tubalmartin/YUI-CSS-compressor-PHP-port

class Minifier extends Beautifier
{
    ...
    public function __construct($raisePhpLimits = true)
    {
        ...
        $this->hexToNamedColorsMap = static::getHexToNamedMap();
        $this->namedToHexColorsMap = static::getNamedToHexMap();
    ...
    {
        $this->memoryLimit = static::normalizeInt($limit);
    }
    ...
    private function doRaisePhpLimits()
    {
        ...
        foreach ($phpLimits as $name => $suggested) {
            $current = static::normalizeInt(ini_get($name));
     ...
     * @return string
     */
    private function runMinifier($css)
    {
        // Process data urls
    ...
    private function shortenHslAndRgbToHexCallback($matches)
    {
        ...
        if ($type === 'hsl') {
            $values = static::hslToRgb($values);
        }

        $hexColors = static::rgbToHex($values);
